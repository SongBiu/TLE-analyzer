...
{
	if (city[cn].n == 0 && cn != 1) return false;//无路可走(可走路数量为0)　　　必须不是最后一个城市！！！
...
//		cout <<"city number" <<t.dest.num  << endl;
...
int main()
{
	...
	for ( i = 1; i <= n; i ++)
		city[i].num = i;
	road = new Road[r+2];//编号从1到r
	for ( i = 0; i <= r; i ++)
		road[i].num = i;
	road[r+1].dest = city[n];road[r+1].lenth = 0;road[r+1].toll = 0;//从road[r+1]开始走
